        ...
        <div class="content">
            <div class="main">
                ...
                <div class="input-and-output">
                    <!--需求文档中的字段输入-->
                    <div class="top">
                        <textarea id="inputField" v-model="inputFieldVal" placeholder="输入需求文档中的字段"></textarea>
                    </div>
                    <!--接口文档中的字段及注释输入-->
                    <div class="top">
                        <textarea
                            id="inputInterface"
                            v-model="inputInterfaceVal"
                            placeholder="输入接口文档中的字段及注释"
                        ></textarea>
                    </div>
                    <!--输出对应格式的代码-->
                    <div class="bottom">
                        <textarea id="output" v-model="outputVal" placeholder="输出对应格式的代码"></textarea>
                    </div>
                </div>
            </div>
            <div class="main log">
...
<script>
import Clipboard from 'src/js/clipboard.js'
import commonReg from 'src/js/commonReg.js'
import homoionymList from 'src/js/homoionymList.js'
import samples from 'src/sample/sample.js'
...
export default {
    ...
    methods: {
        ...
        handleInputVal (callback, type) {
            ...
            if (
                withSeparator &&
                this.inputInterfaceVal.split(separator).length >= fields.length
            ) {
                inputInterfaceValArr = this.inputInterfaceVal
                    .split(separator)
                    .filter(item => item && !separator.test(item))
            } else if (
                withJson &&
                this.inputInterfaceVal.match(commonReg.jsonReg).length >=
                    fields.length
            ) {
                inputInterfaceValArr = this.inputInterfaceVal
                    .match(commonReg.jsonReg)
                    .filter(item => item)
            } else {
                inputInterfaceValArr = this.inputInterfaceVal
                    .split(/\n/g)
                ...
                .map((field, index) => {
                    ...
                    this.getHomoionym(field).map(item => {
                        const regItem = item
                            ...
                            .replace('+', '\\+')
                            .replace('?', '\\?')
                        const matchReg = new RegExp(regItem, 'i')
                        const accurateMatchReg = new RegExp(
                            '(^|[^\u4e00-\u9fa5a-z])' +
                                regItem +
                                '($|[^\u4e00-\u9fa5a-z]|id|名称)',
                            'i'
                        )
                        const accurateMatch = accurateMatchReg.test(
                            this.inputInterfaceVal
                        )
                        inputInterfaceValArr
                            .map(lineStr => {
                                let prop = '' // 字段提取
                                const specailReg = /[,，;\s"'()]/gi // 特俗字符正则
                                const lineStrFilter = lineStr
                                    .replace(/[/*]/gi, '')
                                    .replace(new RegExp('=.*$', 'ig'), '')
                                    .replace(
                                        /(VARCHAR)|(TINYINT)|(DECIMAL)|(DEFAULT)|(UNSIGNED)|(COMMENT)/g,
                                        ''
                                    ) // 文档注释过滤

                                if (
                                    (accurateMatch &&
                                        accurateMatchReg.test(lineStrFilter)) ||
                                    (!accurateMatch &&
                                        matchReg.test(lineStrFilter))
                                ) {
                                    const matches = lineStrFilter.match(
                                        /\b\w+\b/gi
                                    )
                                    if (matches) {
                                        prop = this.matchesSort(
                                            matches,
                                            item,
                                            lineStrFilter
                                        )[0].replace(specailReg, '')
                                        if (
                                            type === 'tableColumns' ||
                                            type === 'export'
                                        ) {
                    ...
                    if (type === 'sourceMap' && !props.length) {
                        const inputInterfaceValMap = this.inputInterfaceVal
                            .replace(/[/*]/gi, '')
                            .split(/[\s,，、|]+/g)
                        if (
                            inputInterfaceValMap.length / fields.length >=
                            0.8
                        ) {
            ...
            if (type && type === 'export') {
                ...
                this.outputVal =
                    (type || 'tableColumns') +
                    ': ' +
                    this.formmatJsonArr(inputValArr)
            }
            this.$notify({
                title: '成功',
                message: '转换成功',
        ...
        getHomoionym (field) {
            ...
            this.homoionymList.map(homoionymItem => {
                homoionymItem.map(word => {
                    if (field.match(word)) {
                        homoionym = word
                        homoionymArr = homoionymItem.filter(
                            item => item !== word
                        )
                    }
                })
        ...
        matchesSort (matches, field, lineStr) {
            ...
            matches.map(item => {
                if (
                    ...
                    (this.type === 'tableColumns' || this.type === 'export') &&
                    /((string|str|name)\b)/gi.test(item)
                ) {
                    weight[item] += 7
                } else if (/\d+\b/gi.test(item)) {
        ...
        getPara (name, url) {
            var paraStr =
                typeof url === 'undefined' ? window.location.href : url
        ...
        setStorage () {
            ...
            window.history.pushState(
                {},
                '',
                '?inputFieldVal=' + encodeURIComponent(this.inputFieldVal)
            )
        },
        // getStorage
        getStorage () {
            if (window.localStorage) {
                this.inputFieldVal =
                    window.localStorage.getItem('inputFieldVal') || ''
                this.inputInterfaceVal =
                    window.localStorage.getItem('inputInterfaceVal') || ''
            }
            if (this.getPara('inputFieldVal')) {
                this.inputFieldVal = this.getPara('inputFieldVal').replace(
                    /&nbsp;/g,
                    ' '
                )
            }
