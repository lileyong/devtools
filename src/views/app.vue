...
export default {
    ...
    methods: {
        ...
        handleInputVal (callback, type) {
            ...
            // 需求字段分割
            const fields = Array.from(new Set(this.inputFieldVal.split(/[\s,，、|]+/g)))
            // 接口文档分割
            ...
            const inputValArr = fields
                ...
                .map((field, index) => {
                    ...
                    this.getHomoionym(field).map(item => {
                        const regItem = item.replace('*', '\\*').replace('+', '\\+').replace('?', '\\?')
                        const matchReg = new RegExp(regItem, 'i')
                        const accurateMatchReg = new RegExp('(^|[^\u4e00-\u9fa5a-z])' + regItem + '($|[^\u4e00-\u9fa5a-z]|id|名称)', 'i')
                        const accurateMatch = accurateMatchReg.test(this.inputInterfaceVal)
                        inputInterfaceValArr.map(lineStr => {
                            let prop = '' // 字段提取
                            const specailReg = /[,，;\s"'()]/ig // 特俗字符正则
                            const lineStrFilter = lineStr.replace(/[/*]/ig, '').replace(new RegExp('=.*$', 'ig'), '').replace(/(VARCHAR)|(TINYINT)|(DECIMAL)|(DEFAULT)|(UNSIGNED)|(COMMENT)/g, '') // 文档注释过滤

                            if ((accurateMatch && accurateMatchReg.test(lineStrFilter)) || (!accurateMatch && matchReg.test(lineStrFilter))) {
                                const matches = lineStrFilter.match(/\b\w+\b/ig)
                                if (matches) {
                    ...
                    if (type === 'sourceMap' && !props.length) {
                        const inputInterfaceValMap = this.inputInterfaceVal.replace(/[/*]/ig, '').split(/[\s,，、|]+/g)
        ...
        matchesSort (matches, field, lineStr) {
            ...
            // 权重优化
            matches.map(item => {
                if (/(\b(private|public|string|int|integer|boolean|body|ApiModelProperty|BigDecimal|\d+|optional)\b)/ig.test(item) || new RegExp('\\.' + item).test(lineStr)) {
                    weight[item] -= 15
